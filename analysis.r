...
f4 <- f4 + geom_point() + labs(x="Product Recalled [log10(pounds)]", y="Produt Recovered [log10(pounds)]")#, title="F4")
...
f5_rca <- ggplot( aes(x = factor(year), y = recalled_log) , data=data) + geom_boxplot(notch=T, size=T)+
  theme_bw() + labs(x = "", y = "Size of Recall [log10(pounds)]", title="A")
f5_rca 
...
f5_pr <- ggplot( aes(x = factor(year), y = Pr_recov_log) , data=data) + geom_boxplot(notch=T, size=T) +
  theme_bw() + labs(x = "", y = "Pr Recovered [log10(Recalled-Rocovered)]", title="C")
f5_pr
...
pf <- pf +  scale_fill_brewer(name="Threshold", labels=c("10^6","10^7","10^8")) + theme(legend.position=c(.1,.85) )
pf
...
pf_bw <- pf + scale_fill_grey(name="Threshold", labels=c("10^6","10^7","10^8"), start=0.8, end = 0.2)
pf_bw
...
figS <- p + labs(x="Year", y="US FSIS Inspected Production [Billion Pounds]", title="E") + theme_bw()
