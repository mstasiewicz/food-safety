...
Xlab <- bquote('Amount Recalled or Recovered ['*log[10]*'(pounds)]') # * = no space, ~ = space; bquote formats 
Ylab <- "Count of Recalls"
f2 <- ggplot(dm, aes(x=value, fill=isZero)) + geom_histogram(binwidth = 0.5) + labs(x=Xlab, y=Ylab, title="Fig 2")
...
#now add in the normal density curve; added post-peer review
dss <- subset(dm, !is.na(value))
dss <- subset(dss, !isZero)
grid <- with(dss, seq(min(value), max(value), length = 100)); binwidth = 0.5
normaldens <- ddply(dss, "variable", function(df) {
  data.frame( 
    predicted = grid,
    value = dnorm(grid, mean(df$value), sd(df$value)) * length(df$value) * binwidth
  )
})
normaldens$isZero = F
f2d <- f2 + geom_line(aes(x = predicted, y = value), data = normaldens, colour = "black") + labs(title="")
#check means 
summary(data$recalled_log)
  f2d + scale_fill_grey( start = 0.3, end = 0.8) + geom_vline(xintercept = 3.799) #means check out!
mean(data$recovered_log[data$recovered_log>0], na.rm = T)
  f2d + scale_fill_grey( start = 0.3, end = 0.8) + geom_vline(xintercept = 3.42) #means check out!
f2d
ggsave(f2d, file="outputs/F2-Hist recall size w nds.pdf")
ggsave(f2d, file="outputs/F2-Hist recall size w nds.jpg")
f2d_bw <- f2d + scale_fill_grey( start = 0.3, end = 0.8)
ggsave(f2d_bw, file="outputs/F2-Hist recall size w nds bw.pdf")
ggsave(f2d_bw, file="outputs/F2-Hist recall size w nds bw.jpg")
...
#some stats follow up to address concerns raised in peer-review####
#other test of log normality 
qqnorm(data$recalled_log) #looks fine to me, even when it fails tests

#skewness and kurtosis.
library(moments)
#other ways to evaluate
summary(data)
skewness(data$recalled_log, na.rm=T) #0.167
kurtosis(data$recalled_log, na.rm=T) #2.88
d <- (subset(data, recalled_log>0 , select=recalled_log) ) #1470 greater than zero
summary(d)
jarque.test( as.vector(t(d)))
#and for recovered 
skewness(subset(data, recovered > 0 , select=recovered_log) )  #0.170
kurtosis(subset(data, recovered > 0 , select=recovered_log) )  #3.21
dim(subset(data, recovered > 0 , select=recovered_log)) #834 records

save.image("recallR.Rdata")

#address recall during 1994-1999 and 2006-2015.
summary(data)
ds <- subset(data, year < 2000 | year > 2005 )
factor(ds$year) #worked
summary(ds$recalled) #median 7,092; mean 560,500
summary(ds$recalled_log) #median 3.851; mean 3.881
10^3.851; 10^3.881
sum(ds$recalled, na.rm=T) #568,903,122
log10(sum(ds$recalled, na.rm=T)) #8.755
#calculate with 0's removed 
summary(subset(ds, recovered_log > 0 , select = recovered_log) ) #mean = 3.422, median 3.438

#investigate sesonal effect
table(data$Season)
chisq.test(table(data$Season)) #no sig
#by species, sig
p <- ggplot(data, aes(x=Season, fill=species)) + geom_bar(position="dodge")
p
ggsave(p, file= "outputs/seasonality-byspecieses.pdf")
#only stec
p %+% (subset(data, reason == "STEC"))

tab <- table(data$Season, data$species)
tab
chisq.test(tab) #sig 0.08; 
tab <- table(subset(data, reason == "STEC", select=Season))
tab
chisq.test(tab) #sig, winter is lower, p < 0.001
tab <- table(subset(data, reason == "STEC", select=c(Season, species)))
addmargins(tab)
ps <- p + facet_wrap(~ reason, scales = "free")
psnf <- p + facet_wrap(~ reason) + theme_bw() + labs(y = "Recall Count")
ggsave(ps, file= "outputs/seasonality-byspecies-pathogen.pdf")
ggsave(psnf, file= "outputs/seasonality-byspecies-pathogen-notfree.pdf")
ggsave(psnf, file= "outputs/seasonality-byspecies-pathogen-notfree.jpg")

#by reason
p <- ggplot(data, aes(x=Season, fill=reason)) + geom_bar(position="dodge")
p  
tab <- table(data$Season, data$reason)
tab
chisq.test(tab) #sig <0.001; 
